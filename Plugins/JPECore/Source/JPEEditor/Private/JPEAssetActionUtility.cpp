...
#include "JPEAssetActionUtility.h"
...
void UJPEAssetActionUtility::RenameSelectedAssets(FString SearchPattern, FString ReplacePattern, ESearchCase::Type SearchCase)
{
...
void UJPEAssetActionUtility::CheckPowerOfTwo()
{
...
bool UJPEAssetActionUtility::IsPowerOfTwo(int32 NumberToCheck)
{
	if (NumberToCheck == 0)
	{
		return false;
...
void UJPEAssetActionUtility::AddPrefixes()
{
	TArray<UObject*> SelectedObjects = UEditorUtilityLibrary::GetSelectedAssets();
...
void UJPEAssetActionUtility::CleanupFolder(FString ParentFolder)
{
...
void UJPEAssetActionUtility::DuplicateAsset(uint32 NumberOfDuplicates, bool bSave)
{
	...
	for (FAssetData AssetData : AssetDataArray)
	{
		for (uint32 i = 0; i < NumberOfDuplicates; ++i)
		{
			...
			if (ensure(UEditorAssetLibrary::DuplicateAsset(AssetData.ToSoftObjectPath().ToString(), NewPath)))
...
void UJPEAssetActionUtility::RemoveUnusedAssets(bool bDeleteImmediately)
{
	TArray<UObject*> SelectedObjects = UEditorUtilityLibrary::GetSelectedAssets();
...
void UJPEAssetActionUtility::PrintToScreen(FString Message, FColor Color)
{
	if (ensure(GEngine))
	{
...
void UJPEAssetActionUtility::GiveFeedback(FString Method, uint32 Counter)
{
