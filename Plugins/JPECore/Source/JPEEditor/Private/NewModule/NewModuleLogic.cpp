...
#define LOCTEXT_NAMESPACE "FLogJPEEditorModule"
...
namespace
{
	EModuleCreationLocation::Type CreateNewModuleInternal(const FString& OutputDirectory, const FModuleDescriptor& NewModule, const FOnCreateNewModuleError& ErrorCallback)
	{
		...
		UE_LOG(LogJPEEditor, Log, TEXT("Creating new module '%s'..."), *NewModule.Name.ToString());
	...
	bool AddNewModuleToUProjectJsonFile(const FModuleDescriptor& NewModule, const FErrorHandler& ErrorHandler)
	{
		...
		{
			UE_LOG(LogJPEEditor, Warning, TEXT("Found multiple .uproject files. Picking '%s'..."), *UProjectFileNames[0]);
		}
	...
	bool AddNewModuleToUPluginJsonFile(const FString& OutputDirectory, const FModuleDescriptor& NewModule, const FErrorHandler& ErrorHandler)
	{
		...
		{
			UE_LOG(LogJPEEditor, Warning, TEXT("Found multiple .uplugin files. Picking '%s'..."), *UPluginFileNames[0]);
		}

		const FString PathToProjectFile = FPaths::Combine(PluginFolderDirectory, UPluginFileNames[0]);
		UE_LOG(LogJPEEditor, Log, TEXT("Path to uplugin file is '%s'..."), *PathToProjectFile);
		return AddNewModuleToFile(PathToProjectFile, NewModule, ErrorHandler);
	}
...
#if ENGINE_MAJOR_VERSION == 4
		bool bCouldReadFile = FFileHelper::LoadFileToString(FileContents, *FullFilePath);
#elif ENGINE_MAJOR_VERSION == 5
		bool bCouldReadFile = FFileHelper::LoadFileToString(FileContents, *FileReadStream.Get());
#endif
		if (!bCouldReadFile)
		{
